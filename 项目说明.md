...
**CaptiPrep = Caption + Preparation（字幕 + 预习）。**
...
├── content.js               # 内容脚本（后端层）：字幕提取、存储、LLM 桥接，暴露 window.CaptiPrep.backend
...
flowchart TB
    ...
    CJ -- "window.CaptiPrep.backend" --> UJ
...
Chrome 内容脚本运行在**隔离世界（isolated world）**，与页面主世界共享 DOM 但不共享 JS 对象。CaptiPrep 需要两类主世界资源：
...
2. 点 "加载已解压的扩展程序" → 选择本目录（`CaptiPrep-main/CaptiPrep-main`）；
...
* 浮层 / 内容脚本：DevTools 的页面 Console（`window.CaptiPrep.backend` 可直接调用）；
