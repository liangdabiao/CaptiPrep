# CaptiPrep 阅读辅助功能融合方案
## —— 基于「掰it」为 CaptiPrep 增加：自动拆长句 / 生词标注 / 手动掰句 / LLM 句式分析

> 参考项目：`bai-it-main`（掰it，v0.2.5，Chrome/Edge/Firefox 英文阅读助手，代码公开、BSL-1.1 许可）
> 目标：把掰it 的四大核心能力**深度融合**进 CaptiPrep（而非简单复制），复用其成熟算法与数据，同时保留 CaptiPrep 已有的学习卡片管线。
...
3. [总体融合架构](#3-总体融合架构)
4. [详细设计](#4-详细设计)
5. [与现有 CaptiPrep 功能的协同](#5-与现有-captiprep-功能的协同)
...
| `content/index.ts`（DOM 扫描/注入） | 需适配 CaptiPrep 架构 | 中 |
| `llm-adapter.ts`（拆句 prompt） | 功能是 CaptiPrep `callProvider` 的子集 | 中（复用现有路由） |
| `db.ts` / options(React) | CaptiPrep 已有存储与单词本 | 高（不移植，走融合） |

> **核心判断：掰it 最有价值、最可复用的是"算法层 + 数据层"（scan-rules / vocab / rule-engine / data 词典），这些是纯逻辑、无框架依赖，可直接以原生 JS 形式并入 CaptiPrep；而它的工程层（IndexedDB 10 表、React 管理端）则融入 CaptiPrep 既有体系，避免双套系统并存。**

---

## 2. 融合目标与设计原则

### 2.1 融合后 CaptiPrep 的能力全景

```
CaptiPrep 融合后 = 阅读辅助（拆句/生词） + 学习管线（卡片/单词本）
                    ↓                      ↓
...
| LLM 深度分析（可选） | 配置 API Key 后，手动掰句升级为 LLM 句式分析（句式分类+讲解+表达+语境释义），**复用 CaptiPrep 现有 LLM 提供商配置** |

### 2.2 设计原则

1. **算法即资产**：移植掰it 打磨过的拆句规则与离线词典，不重造轮子；
2. **LLM 配置零重复**：CaptiPrep 已有 5 家 provider（gemini/openai/anthropic/openrouter/openai-compatible），新增能力直接复用，用户只需配一次 key；
3. **本地优先，LLM 兜底**：默认零配置可用（拆句+词典），配 key 后体验增强；
4. **不破坏页面**：DOM 注入沿用掰it 验证过的"隐藏原元素 + 兄弟插入"策略；
5. **学习沉淀闭环**：阅读中遇到的难句/生词可一键进入 CaptiPrep 学习管线（卡片/单词本），两个功能域共享数据而非割裂。
...
### 移植方式
掰it 的 `scan-rules.ts` 是纯 TypeScript 逻辑（唯一依赖 `pos`，即 pos-js，纯 JS 无依赖）。CaptiPrep 是原生 JS 零构建，**将其转译为原生 JS 模块 `assets/scan-split.js` 直接引入**。
...
| 多语言门控 | CaptiPrep 是多语言产品（9 种学习语言）；拆句/标注仅作用于**英文**（`isEnglish` 检测，阈值 0.5），非英文文本跳过，避免误伤日/中/韩阅读 |
| 拆句强度配置 | 复用掰it 的 `chunkIntensity`（1-5）与 `chunkGranularity`（coarse/medium/fine），写入 CaptiPrep settings |

## 4.2 生词标注模块（移植 `vocab.ts` + 离线词典）

### 数据资产（直接搬运）

| 文件 | 大小 | 作用 | CaptiPrep 落位 |
| --- | --- | --- | --- |
...
掰it 用 esbuild 把 JSON `import` 进 bundle；CaptiPrep 无构建，采用**异步加载 + 内存缓存**：
...
1. **白名单选择器**：X/Twitter、Reddit、Medium、`article p`、`main p`、`.content p` 等（适配 CaptiPrep 的站点矩阵，可配置扩展）；
...
### 与 CaptiPrep 现有 content script 的共存
...
## 4.5 LLM 深度分析（复用 CaptiPrep 现有路由）

### 关键决策：不移植掰it 的 llm-adapter，直接复用 `background.js` 的 `callProvider`

CaptiPrep 已支持 5 家 provider 的统一路由（gemini / openai / anthropic / openrouter / openai-compatible），掰it 的 llm-adapter 只是其中 2 家的子集。融合方式：

```
background.js 新增消息：CC_SENTENCE_ANALYSIS
  ├─ 参数：{ sentence, role: 'chunk' | 'full' }
  ├─ 用 CaptiPrep 现有 settings(provider/apiKey/model) 解析出 LLMConfig
...
### 两个新增 Prompt（移植自掰it，适配 CaptiPrep 风格）
...
CaptiPrep 现有浮层 + options 页可承载以下新设置：
...
**Popup 交互**（借鉴掰it）：大按钮"掰it/显示原文"切换当前页 + 站点开关 + 辅助力度滑杆 + 显示方式分段选择器（实时预览）。CaptiPrep 可把这些做进浮层或独立 popup。
...
**协同收益**：
- CaptiPrep 已有的 LLM 提供商体系（用户若配过 key）被新功能直接复用，无重复配置；
- 阅读时顺手收藏的生词/难句自动进入单词本，形成"阅读 → 学习"闭环；
- knownWords 让 LLM#1 的候选筛选越来越精准（与 CaptiPrep 现有 `alignCardsToSelected` 等后处理互补）。
...
| `options`(React) / `db.ts`(10表) | **不移植** | 融入 CaptiPrep 单词本与既有存储 |
| `background/index.ts` | 不移植 | 复用 CaptiPrep `callProvider` 路由 |
...
> 该方案的关键价值：**用最小成本（复用成熟算法 + 复用现有 LLM 路由）让 CaptiPrep 具备"任意英文网页阅读辅助"能力，同时不破坏其 YouTube 学习管线，并最终通过"难句/生词 → 学习卡片"实现阅读与学习的闭环。**
